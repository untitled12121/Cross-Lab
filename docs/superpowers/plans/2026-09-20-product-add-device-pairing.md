...
## Task 1 — Product invitation presentation — implemented
...
## Task 2 — Android QR scanner — implemented
...
## Task 4 — Local pairing discovery/transport — implemented
...
- Linux uses bounded Avahi advertisement; Android uses bounded NsdManager resolution after a validated QR bootstrap.
- The shared product pairing coordinator runs over the provisional Quinn channel with explicit persistence acknowledgements before product-visible success.
- Linux GPUI and Android Compose expose the real pairing lifecycle and terminal retry/dismiss states.
...
- Pairing coordinator/network integration tests covering success, cancellation, replay, malformed bootstrap, proof failure, persistence failure, message ordering/final acknowledgements, and rollback/currentness failure.
