...
A successful destination response contains:

- the same `TransferId`;
- a destination-validated `resume_offset`;
- a fresh `OperationId` authorizing one source-to-destination data stream.

Before returning success, the destination:

1. verifies negotiated `files.transfer` v2.0 and exact local `receive` policy;
2. validates or creates bounded partial-transfer state for the offered file identity;
...
v2.0 resumes only from the destination's **largest verified contiguous prefix**.
...
The destination may persist only bounded transfer metadata needed to recover a partial transfer:
...
- verified contiguous offset;
- platform-local partial-file locator.

The platform-local locator is never sent over the network and is not identity or authorization material.

After interruption, the source repeats the transfer offer using the same `TransferId`, size, name, and digest. The destination returns the last durable verified checkpoint. A changed identity tuple is rejected rather than merged with the previous partial file.
...
Partial file metadata may remain for a later explicit resume, but retained partial state carries no active session authority.
...
- versioned bounded offer/accept bodies;
- 256-bit `TransferId`;
...
Partial-transfer persistence is capability state, separate from identity/policy stores, and must have bounded cleanup/expiry rules before product rollout.

## Consequences

Cross-Lab gets a small resumable transfer model that reuses existing authenticated control and authorized data-stream boundaries instead of creating a second transfer transport.
