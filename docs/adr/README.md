...
Proposed architecture decisions awaiting written-spec review:

| ADR | Decision | Status |
|---|---|---|
| ADR-0010 | Authoritative active-root/delegated-role currentness and fail-closed session invalidation | Proposed |
| ADR-0011 | Bounded `RequestId` retry/correlation state with `SessionId` + `message_seq` as session anti-replay | Proposed |
