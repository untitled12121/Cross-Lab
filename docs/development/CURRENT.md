...
- Active Phase 2 branch: `phase2-device-permissions`.
...
1. implement the Phase 2 per-device permissions/capability-control vertical slice on the existing authenticated session/policy boundaries;
2. keep capability support separate from permission and preserve default-deny/no-rule behavior;
3. expose only presentation-safe per-device capability/permission state in Linux GPUI and Android Compose;
4. then continue through clipboard, resumable file transfer, notifications, audit/history, and revocation/device removal;
