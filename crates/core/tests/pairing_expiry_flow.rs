use crosslab_core::{
    PairingFlowError, PairingId, PairingInstant, PairingInvitation, PairingInvitationState,
    PairingInviterFlow, PairingInviterState, PairingJoinerFlow, PairingSecret,
};
use crosslab_crypto::SigningKey;
use crosslab_identity::{
    AuthorityDelegation, AuthorityRole, DeviceId, OwnerAuthorityState, OwnerId, OwnerRootRecord,
};
use crosslab_policy::TransitionId;
use crosslab_protocol::{PairingHello, PairingRole};

#[test]
fn invitation_expiry_mid_flow_prevents_trust_commit() {
    let owner_id = OwnerId::from_bytes([0x51; 32]);
    let root_key = SigningKey::from_secret_bytes([0x52; 32]);
    let root = OwnerRootRecord::new(owner_id, &root_key, 0);
    let issuer_key = SigningKey::from_secret_bytes([0x53; 32]);
    let delegation = AuthorityDelegation::issue(
        owner_id,
        AuthorityRole::DeviceSigning,
        &issuer_key,
        0,
        &root_key,
    );
    let mut authority = OwnerAuthorityState::new(root);
    authority.accept_delegation(delegation).unwrap();
    let inviter_key = SigningKey::from_secret_bytes([0x54; 32]);
    let joiner_key = SigningKey::from_secret_bytes([0x55; 32]);
    let inviter_device_id = DeviceId::from_bytes([0x56; 32]);
    let joiner_device_id = DeviceId::from_bytes([0x57; 32]);
    let pairing_id = PairingId::from_bytes([0x58; 16]);
    let secret = [0x59; 32];
    let inviter_hello = PairingHello::new(
        PairingRole::Inviter,
        1,
        pairing_id.to_bytes(),
        owner_id,
        inviter_device_id,
        inviter_key.verifying_key(),
        [0x5a; 32],
    );
    let joiner_hello = PairingHello::new(
        PairingRole::Joiner,
        1,
        pairing_id.to_bytes(),
        owner_id,
        joiner_device_id,
        joiner_key.verifying_key(),
        [0x5b; 32],
    );
    let invitation = PairingInvitation::from_parts(
        pairing_id,
        PairingSecret::from_bytes(secret),
        owner_id,
        inviter_device_id,
        PairingInstant::from_ticks(10),
        PairingInstant::from_ticks(20),
    )
    .unwrap();
    let mut inviter = PairingInviterFlow::new(
        invitation,
        inviter_hello,
        joiner_hello,
        PairingInstant::from_ticks(10),
    )
    .unwrap();
    let mut joiner = PairingJoinerFlow::new(
        PairingSecret::from_bytes(secret),
        inviter_hello,
        joiner_hello,
    )
    .unwrap();

    let joiner_confirmation = joiner.joiner_confirmation().unwrap();
    let inviter_confirmation = inviter
        .verify_joiner_confirmation(&joiner_confirmation, PairingInstant::from_ticks(15))
        .unwrap();
    joiner
        .verify_inviter_confirmation(&inviter_confirmation)
        .unwrap();
    let credential = inviter
        .issue_initial_joiner_credential(&authority, &issuer_key, PairingInstant::from_ticks(19))
        .unwrap();
    let accepted = joiner
        .accept_credential(&authority, &credential, &joiner_key)
        .unwrap();

    assert_eq!(
        inviter.commit_trust(
            &accepted,
            TransitionId::from_bytes([0x5c; 32]),
            &authority,
            &issuer_key,
            PairingInstant::from_ticks(20),
        ),
        Err(PairingFlowError::InvitationExpired)
    );
    assert_eq!(inviter.state(), PairingInviterState::Failed);
    assert_eq!(inviter.invitation_state(), PairingInvitationState::Expired);
}
